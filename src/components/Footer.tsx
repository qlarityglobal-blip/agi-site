...
export function Footer() {
  return (
    <footer className="relative overflow-hidden bg-charcoal text-aluminium-light">
        ...
        <div className="grid gap-12 lg:grid-cols-[1.3fr_1fr_1fr_1.2fr]">
          <div>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <span className="inline-flex items-center gap-1.5 rounded-full border border-bronze/40 bg-bronze/10 px-3 py-1.5 text-xs font-semibold text-bronze">
                <Award size={13} />
                B-BBEE Level 2
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 px-3 py-1.5 text-xs font-medium text-aluminium-light">
                <ShieldCheck size={13} />
                CIDB Registered
              </span>
            </div>
          ...
          <div>
            <h4 className="font-heading text-sm font-semibold uppercase tracking-widest text-white">
              Navigate
            </h4>
            <ul className="mt-5 space-y-3 text-sm">
              {nav.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="text-aluminium transition hover:text-bronze">
                    {item.label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3 text-sm">
              {services.slice(0, 6).map((s) => (
                <li key={s.slug} className="text-aluminium">
                  {s.name}
                </li>
              ))}
            </ul>
            ...
            <ul className="mt-5 space-y-4 text-sm text-aluminium">
              ...
              {contact.phones.map((p) => (
                <li key={p.label} className="flex gap-3">
                  <Phone size={18} className="mt-0.5 shrink-0 text-bronze" />
                  <span>
              ...
              <li className="flex gap-3">
                <Mail size={18} className="mt-0.5 shrink-0 text-bronze" />
